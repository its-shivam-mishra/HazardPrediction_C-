    ...
    public class HazardService : IHazardService
    {
        ...
        public async Task<HazardPrediction> PredictHazardsAsync(UnifiedWeatherResponse weatherData)
        {
            ...
            try
            {
                ...
                var prompt = $@"
                                You are a weather hazard expert. Analyze the following weather data for {weatherData.City} and predict the risk percentage (0-100) for various hazards, Include a hazard in the output only if its predicted risk percentage is greater than 0.
